#!/bin/bash
# Set repo
export BORG_REPO='ssh://USER@example.com:PORT/path/to/vaultwarden_data_repo'

# Stop container
/usr/bin/docker-compose -f /path/to/compose.yml --project-directory /path/to/ down

# Run backup
/usr/bin/borg create --stats ::$(hostname)-$(date -I) /path/to/docker/volumes/vaultwarden_data/
